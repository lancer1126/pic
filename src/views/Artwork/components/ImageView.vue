  ...
  >
    ...
    >
      ...
      <img v-else
           ...
           height: ((artwork.width / displayWidth) * artwork.height) * (artwork.width / artwork.height)}"/>
      <canvas v-if="artwork.type==='ugoira'"
              class="ugoira"
              id="ugoira" ref="ugoira"
              :width="artwork.width"
  ...
  methods: {
    ...
    async playUgoira () {
      ...
      ugoira.frames.forEach(frame => {
        frames[frame.file] = false
      })

      this.ugoira = { frames, zip: ugoira.zip }
      this.progressShow = true

      axios.get(ugoira.zip, {
            responseType: 'blob',
            timeout: 1000 * 30,
            onDownloadProgress: progress => {this.progress = progress.loaded / progress.total}})
          .then(resp => {
